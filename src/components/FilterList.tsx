...
export default function FilterList(props: IFilerListProps) {
	...
	return (
		<>
			...
			<div className="py-4 flex flex-row flex-wrap gap-2 items-center">
				<button
				...
				{
					props.yearRange &&
					<div className="flex justify-center items-center border border-gray-400 dark:border-slate-600 border-dashed py-1 px-4 h-fit rounded-full">
				...
				}
				{
					props.genres.map(genre => {
						return (
							<div key={genre.key} className="flex justify-center items-center border border-gray-400 dark:border-slate-600 border-dashed py-1 px-4 h-fit rounded-full">
								{genre.value}
							</div>
						);
					})
